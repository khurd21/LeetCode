...
Each person should appear in exactly one group, and every person must be in a group. If
there are multiple answers, return any of them. It is guaranteed that there will be at
...
public:
    std::vector<std::vector<int>> groupThePeople(const std::vector<int>& groupSizes) {
        ...
        for (int personId{}; personId < groupSizes.size(); ++personId) {
            const int sizeOfGroup{ groupSizes[personId] };
            grouped[sizeOfGroup].push_back(personId);

            auto& group{ grouped[sizeOfGroup] };

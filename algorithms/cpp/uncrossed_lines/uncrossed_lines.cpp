 ...
 * 
 * nums1[i] == nums2[j], and the line we draw does not intersect any
 ...
 * We cannot draw 3 uncrossed lines, because the line from nums1[1] = 4
 * to nums2[2] = 4 will intersect the line from nums1[2]=2 to nums2[1]=2.
 * 
 * Example 2:
 * 
    ...
    int maxUncrossedLines(const std::vector<int>& nums1, const std::vector<int>& nums2) {
        ...
        for (int i = 1; i < nums1.size() + 1; ++i) {
            for (int j = 1; j < nums2.size() + 1; ++j) {
                dp[i][j] = nums1[i-1] == nums2[j-1] ? dp[i-1][j-1] + 1 : std::max(dp[i][j-1], dp[i-1][j]);
            }
        }

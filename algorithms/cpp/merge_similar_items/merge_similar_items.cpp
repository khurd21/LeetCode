 ...
 * 
 * The item with value = 4 occurs in items1 with weight = 5,
 * total weight = 5.  
 ...
 * items2 with weight = 2, total weight = 2 + 2 = 4. 
 * 
 * The item with value = 7 occurs in items2 with weight = 1, total
 * weight = 1.
 * 
...
public:
    std::vector<std::vector<int>> mergeSimilarItems(
        const std::vector<std::vector<int>>& items1,
...
private:
    ...
    void sum(const std::vector<std::vector<int>>& v, std::map<int, int>& m) {

        for (const std::vector<int>& item : v) {
            const int value{ item[0] };
            const int weight{ item[1] };

            m[value] = m.find(value) == m.end() ? weight : weight + m[value];
        }
    }
};
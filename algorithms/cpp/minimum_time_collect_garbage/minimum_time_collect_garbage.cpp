...
'P' and 'G' representing one unit of metal, paper and glass garbage respectively. Picking
up one unit of any type of garbage takes 1 minute.
...
public:
    int garbageCollection(const std::vector<std::string>& garbage, const std::vector<int>& travel) {
        ...
        auto travel_itr{ travel.end() };
        bool is_g{};
        bool is_p{};
        bool is_m{};
        for (auto itr = garbage.rbegin(); itr < garbage.rend(); ++itr) {
            result += itr->size();
            if (!is_g && std::find(itr->begin(), itr->end(), 'G') != itr->end()) {
                is_g = true;
                result += std::accumulate(travel.begin(), travel_itr, 0);
            }
            if (!is_p && std::find(itr->begin(), itr->end(), 'P') != itr->end()) {
                is_p = true;
                result += std::accumulate(travel.begin(), travel_itr, 0);
            }
            if (!is_m && std::find(itr->begin(), itr->end(), 'M') != itr->end()) {
                is_m = true;
                result += std::accumulate(travel.begin(), travel_itr, 0);
            }
            --travel_itr;

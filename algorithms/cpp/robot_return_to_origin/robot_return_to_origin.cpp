...
class Solution {
public:
    bool judgeCircle(const std::string& moves) const {

        std::uint32_t plane{ 0 };
        return is_origin_plane(moves, 'U', 'D', plane) && is_origin_plane(moves, 'L', 'R', plane);
    }

private:

    static inline bool is_origin_plane(const std::string& moves, const char incrementor, const char decrementer, std::uint32_t& plane) {

        for (const char move : moves) {
            plane += move == incrementor ? 1 : 0;
            plane += move == decrementer ? -1 : 0;
        }
